...
if [ ! -f "$VENV/.deps-ok" ]; then
    ...
    # torch is installed separately on purpose: the CUDA wheels do not live on
    # PyPI, so they cannot be pinned in the requirements file.
    if command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L >/dev/null 2>&1; then
        log "NVIDIA GPU detected, installing torch $TORCH_CUDA"
        "$VPY" -m pip install --quiet torch torchvision \
            --index-url "https://download.pytorch.org/whl/$TORCH_CUDA"
    else
        printf '\n\033[33mWARNING: no NVIDIA GPU found. Installing the CPU build.\n'
        printf 'The full grid is not practical on a CPU: use test_config.json instead.\033[0m\n\n'
        "$VPY" -m pip install --quiet torch torchvision
    fi
    touch "$VENV/.deps-ok"
else
